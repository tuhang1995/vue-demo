...
router.get('/delete', function(req, res) {
  ...
  var sql = `delete from study where id='${value}'`
...
router.post('/login', (req, res) => {
  ...
  db.query('select * from user', (err, data) => {
    ...
    if (arr[0].userName == userName) {
      bcrypt
        .compare(password, arr[0].password)
        .then((isMatch) => {
          //如果密码正确
          if (isMatch) {
            const rule = {
...
router.post('/add', (req, res) => {
  ...
  var flag = true // true表示用户名可用 false 表示用户名重复了不能使用
  var addSql =
    'INSERT INTO study(name,area,time,resources,content,purpose) VALUES(?,?,?,?,?,?)'
  var addSqlParams = [
    req.body.name,
    req.body.area,
    req.body.time,
...
router.get('/search', (req, res) => {
  ...
  db.query(
    `SELECT * FROM study where name like '%${value}%'`,
    (qerr, data, fields) => {
      res.end(
        res.json({
          status: 0,
